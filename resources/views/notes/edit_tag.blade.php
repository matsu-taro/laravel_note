      ...
      <form action="{{ route('notes.store') }}" method="post">
        ...
        <div class="note__box--tag">
          ...
          <select name="tag_id">
            ...
            <option value="1" @if($tag==1) selected @endif {{ old('tag_id')==1 ? 'selected' : '' }}>仕事</option>
            <option value="2" @if($tag==2) selected @endif {{ old('tag_id')==2 ? 'selected' : '' }}>遊び</option>
            <option value="3" @if($tag==3) selected @endif {{ old('tag_id')==3 ? 'selected' : '' }}>その他</option>
          </select>
        </div>
    ...
    <div class="note__area--bottom">
      ...
      <div class="note__box-memos">
        <p class="note--title">
          ...
          @if($tag == 2) 遊び @endif
          @if($tag == 3) その他 @endif
          」のメモ一覧 <span>(選択すると内容の確認と編集ができるよ)</span>
        </p>
        <ul>
          @foreach($memos as $memo)
          <li>
            <a class="text-blue-400 memo-list" href="{{ route('notes.edit',['id' => $memo->id]) }}">
              ・{{ $memo['title'] }}
            </a>
            ...
            <div class="gomibako">
              <form action="{{ route('notes.dust' , ['id' => $memo->id]) }}" method="post">
                @csrf
                <button name="dust_item" value="{{ $memo->status }}">
